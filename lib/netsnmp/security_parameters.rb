  ...
  class SecurityParameters
    ...
    def check_parameters
      ...
      if @security_level > 0
        ...
        raise Error, "security level requires an auth password" if @auth_password.nil?
        raise Error, "auth password must have between 8 to 32 characters" if not (8..32).include?(@auth_password.length)
      end
      if @security_level > 1
        @priv_protocol ||= :des
        raise Error, "security level requires a priv password" if @priv_password.nil?
        raise Error, "priv password must have between 8 to 32 characters" if not (8..32).include?(@priv_password.length)
      end
    end
